...
PYROSCOPE_FLUSH_INTERVAL_MS
PYROSCOPE_WALL_COLLECT_CPU_TIME
```

Node/container services use OpenTelemetry for metrics and traces, Loki for
structured logs, and Pyroscope for continuous profiling. Cloudflare Workers do
not receive these Node-only profiler credentials; they use Workers Observability
and Analytics Engine instead.
