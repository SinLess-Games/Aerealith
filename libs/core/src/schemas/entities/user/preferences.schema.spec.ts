...
describe('preference enum schemas', () => {
  ...
  it('rejects unsupported preference enum values', () => {
    ...
    expect(NameDisplayOrderSchema.safeParse('invalid').success).toBe(false)
    ...
    expect(TimezoneGreenwichSchema.safeParse('invalid').success).toBe(false)
...
describe('UserPreferencesEntitySchema', () => {
  it('accepts a complete internal user preferences entity', () => {
    ...
    if (result.success) {
      ...
      expect(result.data.timezoneGreenwich).toBe(timezoneGreenwich)
