...
describe('profile field schemas', () => {
  ...
  it('trims a valid profile bio', () => {
    const result = UserProfileBioSchema.safeParse('  Building Aerealith.  ')
  ...
  it('trims a valid profile location', () => {
    const result = UserProfileLocationSchema.safeParse('  Twin Falls, Idaho  ')
  ...
  it('trims and accepts a valid profile URL', () => {
    const result = UserProfileUrlSchema.safeParse('  https://aerealith.com  ')
...
describe('UserProfileContractSchema', () => {
  ...
  it('accepts a complete owner or admin profile contract', () => {
    ...
    if (result.success) {
      ...
      expect(result.data.bio).toBe('Building Aerealith.')
      expect(result.data.locationLabel).toBe('Twin Falls, Idaho')
      expect(result.data.createdAt).toBe('2026-06-20T12:00:00.000Z')
      expect(result.data.updatedAt).toBe('2026-06-20T12:10:00.000Z')
    }
  })
