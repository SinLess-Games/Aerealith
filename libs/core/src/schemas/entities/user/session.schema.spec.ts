...
describe('user session field schemas', () => {
  ...
  it('rejects invalid session and user IDs', () => {
    ...
    expect(UserSessionUserIdSchema.safeParse('not-a-uuid').success).toBe(false)
  })

  it('accepts a valid token hash', () => {
    const result = UserSessionTokenHashSchema.safeParse('hashed-session-token')
  ...
  it('trims a valid user agent', () => {
    const result = UserSessionUserAgentSchema.safeParse('  Mozilla/5.0  ')
