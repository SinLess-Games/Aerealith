...
describe('UserSessionEntity', () => {
  ...
  it('creates a session with safe defaults', () => {
    ...
    expect(session.lastSeenAt).toEqual(
  ...
  it('reports an expired session correctly', () => {
    ...
    expect(session.isExpired(new Date('2026-06-20T12:00:00.000Z'))).toBe(
      true,
    );
  });
  ...
  it('revokes a session and updates its timestamp', () => {
    ...
    expect(session.revokedAt).toEqual(
      new Date('2026-06-20T12:15:00.000Z'),
    );
  ...
  it('keeps soft-delete behavior from BaseEntity', () => {
    ...
    expect(session.deletedAt).toEqual(
