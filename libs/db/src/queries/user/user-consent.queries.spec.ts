...
describe('user consent queries', () => {
  ...
  it('creates an active user consent predicate by user ID and type', () => {
    ...
    expect(query.params).toEqual(['user_123', UserConsentType.PrivacyPolicy])
  ...
  it('uses the provided user ID and consent type as predicate parameters', () => {
    ...
    const query = toQuery(activeUserConsentByUserIdAndType(userId, type))
  ...
  it('always excludes soft-deleted user consents', () => {
    ...
    const byUserIdQuery = toQuery(activeUserConsentsByUserId('user_123'))

    expect(byIdQuery.sql).toContain('"user_consents"."deleted_at" is null')

    expect(byUserIdAndTypeQuery.sql).toContain(
      '"user_consents"."deleted_at" is null',
    )

    expect(byUserIdQuery.sql).toContain('"user_consents"."deleted_at" is null')
  })
})
