...
describe('user queries', () => {
  ...
  it('creates an active user predicate by normalized email', () => {
    const query = toQuery(
      activeUserByEmail('  ANDY@EXAMPLE.COM  '),
    );
  ...
  it('creates an active user predicate by normalized username', () => {
    const query = toQuery(
      activeUserByUsername('  SinLess777  '),
    );
  ...
  it('normalizes equivalent email addresses into the same predicate', () => {
    const lowerCaseQuery = toQuery(
      activeUserByEmail('andy@example.com'),
    );

    const mixedCaseQuery = toQuery(
      activeUserByEmail('AnDy@ExAmPlE.cOm'),
    );
  ...
  it('always excludes soft-deleted users', () => {
    ...
    const byEmailQuery = toQuery(
      activeUserByEmail('andy@example.com'),
    );

    const byUsernameQuery = toQuery(
      activeUserByUsername('sinless777'),
    );

    expect(byIdQuery.sql).toContain('"users"."deleted_at" is null');

    expect(byEmailQuery.sql).toContain(
      '"users"."deleted_at" is null',
    );

    expect(byUsernameQuery.sql).toContain(
      '"users"."deleted_at" is null',
    );
  });
});
