...
import {
  ...
  authenticationEligibleUserById,
} from './user.queries';
...
describe('user queries', () => {
  ...
  it('requires active, verified, non-deleted users for session authentication', () => {
    const query = toQuery(authenticationEligibleUserById('user_123'));

    expect(query.params).toEqual(['user_123', 'active', true]);
    expect(query.sql).toContain('"users"."status" = $2');
    expect(query.sql).toContain('"users"."email_verified" = $3');
    expect(query.sql).toContain('"users"."email_verified_at" is not null');
    expect(query.sql).toContain('"users"."deleted_at" is null');
  });
