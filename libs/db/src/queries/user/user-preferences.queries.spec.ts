...
describe('user preferences queries', () => {
  it('creates an active user preferences predicate by user ID', () => {
    ...
    expect(query.sql).toContain('"user_preferences"."deleted_at" is null')
  })
  ...
  it('creates independent predicates for different users', () => {
    const firstQuery = toQuery(activeUserPreferencesByUserId('user_123'))

    const secondQuery = toQuery(activeUserPreferencesByUserId('user_456'))
