...
function toQuery(
  condition: ReturnType<typeof activeWaitlistEntryByEmail>,
) {
...
describe('waitlist queries', () => {
  it('creates an active waitlist entry predicate by email', () => {
    const query = toQuery(
      activeWaitlistEntryByEmail('andy@example.com'),
    );
  ...
  it('creates independent predicates for different email addresses', () => {
    const firstQuery = toQuery(
      activeWaitlistEntryByEmail('first@example.com'),
    );
  ...
  it('always excludes soft-deleted waitlist entries', () => {
    ...
    expect(query.sql).toContain(
      '"waitlist_entries"."deleted_at" is null',
    );
  });
});
