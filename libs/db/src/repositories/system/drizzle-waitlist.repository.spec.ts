...
function createWaitlistRow(overrides: Partial<WaitlistRow> = {}): WaitlistRow {
  return {
    id: 'waitlist_123',
...
describe('DrizzleWaitlistRepository', () => {
  it('finds an active waitlist entry by email', async () => {
    ...
    const repository = new DrizzleWaitlistRepository(databaseMock.database)
  ...
  it('soft deletes an existing waitlist entry', async () => {
    ...
    const result = await repository.softDeleteByEmail('person@example.com')
  ...
  it('returns false when there is no waitlist entry to soft delete', async () => {
    ...
    const result = await repository.softDeleteByEmail('missing@example.com')
